...
class DatabaseViewer(QDialog):
    ...
    def setup_dialog(self):
        ...
        self.setFixedSize(UISizes.DATABASE_VIEWER_WIDTH, UISizes.DATABASE_VIEWER_HEIGHT)
        self.setModal(True)
    ...
    def setup_tableView(self):
        ...
        self.tableView_expInfoDB.horizontalHeader().setDefaultAlignment(UIAlignments.CENTER)
        self.tableView_expInfoDB.verticalHeader().setVisible(False)
        self.tableView_expInfoDB.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeToContents)
        self.tableView_expInfoDB.setItemDelegate(customized_delegate.CenterAlignDelegate())
    ...
    def load(self):
        ...
        df = pd.read_sql_query(f"SELECT * FROM {self.selected_table} WHERE id = {primary_key}", conn)
        conn.close()
        ...
        if df["Enable_R"][0]=="True":
            ...
            self.ui.checkBox_enable_R.setCheckState(Qt.Unchecked)
            self.main.groupBox_R_available(Qt.Unchecked)
        
        if df["Enable_L"][0]=="True":
            self.ui.checkBox_enable_L.setCheckState(Qt.Checked)
    ...
    def delete(self):
        ...
        checkDeletion = dialog_confirm.Confirm(title="Checking...", msg="Delete selected rows?")
        ...
        if self.model_expInfoDB.submitAll():
            ...
            print("[bold red]Failed to delete data from database! The model is reverted![/bold red]")
    ...
    def export(self):
        ...
        dlg_get_outputDir.setWindowTitle("Select the output directory of the markdown file of selected expinfo!")
        ...
        with open(os.path.join(dir_output, df['DOR'][0] + '_expInfo.md'), 'w', encoding="utf-8") as f:
            f.write("---\n")
