...
class ModelMetadataForm(QAbstractTableModel):
    ...
    def data(self, index, role):
        # When displaying data one by one (no need actual loop), render them according to their types.
    ...
    def mv_rows(self, rows, step):
        ...
        if target_row == 0:
            ...
            self._data = pd.concat([self._data.iloc[:target_row], selected_df, self._data.iloc[target_row:]])
            new_indices = remaining_indices[:target_row] + selected_indices + remaining_indices[target_row:]
    ...
    def add_rows(self, SourceParent, start_from_this_row, dataFrameToBeAdded):
        ...
        self._data = pd.concat(
            [self._data.iloc[: start_from_this_row + 1], dataFrameToBeAdded, self._data.iloc[start_from_this_row + 1 :]]
        ).reset_index(drop=True)
        self._data.index = (
            rowNames[: start_from_this_row + 1]
            + dataFrameToBeAdded.index.tolist()
            + rowNames[start_from_this_row + 1 :]
        )
