...
The project is a GUI tool for recording metadata of the tiff and abf files during the experiment and generate tags for writing to the recording files (*.rec). It is written in Python and uses PySide6 for the GUI. Now that the primary functions are all implemented, I'm going to add analysis functions/panels which properly utilize the databases (rec_data.db) built by the GUI tool.

## Documentation
1. When create new markdown files (except README.md), always saved in the `docs/` folder. If `docs/` does not exist, create one.
2. When user using plan mode creates new markdown plans, always saved in the `.claude/plans/` folder. If `plans/` does not exist, create one.
