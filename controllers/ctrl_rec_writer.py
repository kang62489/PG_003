...
class CtrlRecWriter:
    ...
    def connect_signals(self):
        ...
        self.ui.comboBox_EXC.activated.connect(self.update_tag_output)
        self.ui.comboBox_EXC.currentIndexChanged.connect(self.auto_select_emi)
        self.ui.lineEdit_LEVEL.textChanged.connect(self.update_tag_output)
        self.ui.lineEdit_EXPO.textChanged.connect(self.update_tag_output)
        self.ui.comboBox_EXPO_UNITS.activated.connect(self.update_tag_output)
        self.ui.comboBox_EMI.activated.connect(self.update_tag_output)
        self.ui.lineEdit_FRAMES.textChanged.connect(self.update_tag_output)
        self.ui.lineEdit_FPS.textChanged.connect(self.update_tag_output)
        self.ui.spinBox_SLICE.valueChanged.connect(self.update_tag_output)
        self.ui.comboBox_CAM_TRIG_MODES.activated.connect(self.update_tag_output)
        self.ui.spinBox_SLICE.valueChanged.connect(self.update_tag_output)
        self.ui.comboBox_LOC_TYPES.activated.connect(self.update_tag_output)
        self.ui.spinBox_AT.valueChanged.connect(self.update_tag_output)
        self.ui.checkBox_addCustomized.stateChanged.connect(self.update_tag_output)

        # Emit signal to update tag output when the table in the QTableView (tableView_customized) is changed
        self.model_tableView_customized.dataChanged.connect(self.update_tag_output)
        self.model_tableView_customized.layoutChanged.connect(self.update_tag_output)

        self.ui.comboBox_tagTemplates.activated.connect(self.template_load)
        self.ui.btn_saveTemplate.clicked.connect(self.template_save)
        self.ui.btn_deleteCurrentTemplate.clicked.connect(self.template_delete)
        self.ui.btn_addNewRows.clicked.connect(self.insert_customized_properties)
        self.ui.btn_removeSelectedRows.clicked.connect(self.rm_selected_rows)
        self.ui.btn_moveUp.clicked.connect(self.mv_rows_up)
        self.ui.btn_moveDown.clicked.connect(self.mv_rows_down)

        self.ui.lineEdit_recDir.textChanged.connect(self.validate_rec_dir)
        self.ui.btn_browse.clicked.connect(self.browse_rec_dir)

        self.ui.lineEdit_filenameSN.textChanged.connect(self.sn_validate)

        self.ui.btn_increaseSN.clicked.connect(self.sn_inc)
        self.ui.btn_decreaseSN.clicked.connect(self.sn_dec)
        self.ui.btn_resetSN.clicked.connect(self.sn_reset)
        self.ui.btn_copyFilenameSN.clicked.connect(self.sn_copy)

        self.ui.btn_writeToRec.clicked.connect(self.write_rec)
        self.ui.btn_loadFromRec.clicked.connect(self.read_rec)
        self.ui.btn_recoverRec.clicked.connect(self.revert_rec)
    ...
    def update_tag_output(self):
        ...
        if self.ui.checkBox_addCustomized.isChecked():
            for prop, val in zip(
                self.model_tableView_customized._data.index.tolist(),
                self.model_tableView_customized._data["VALUE_0"].tolist(),
            ):
                props.append(prop)
                values.append(val)
    ...
    def template_save(self):
        ...
        self.saveDialog.savefile(
            os.path.join(MODELS_DIR), self.model_tableView_customized._data
        )
    ...
    def template_delete(self):
        ...
        if self.deleteCheck.exec():
            ...
            self.template_reload_list()
            self.clear_qtable_view(self.model_tableView_customized)
            self.ui.btn_deleteCurrentTemplate.setEnabled(False)
        else:
    ...
    def insert_customized_properties(self):
        ...
        self.model_tableView_customized.add_rows(
            QModelIndex(), insert_from_this_row, self.dlg_insertion.dataToBeAdded
        )
        new_index = self.model_tableView_customized.index(
    ...
    def rm_selected_rows(self):
        ...
        for row in rows_to_be_removed:
            self.model_tableView_customized.rm_rows(QModelIndex(), row, 1)
    ...
    def mv_rows_up(self):
        ...
        # Move rows up
        new_positions_of_moved_rows = self.model_tableView_customized.mv_rows(rows, -1)
    ...
    def backup_rec_contents(
        ...
        with open(backup_path, mode="w") as f:
            ...
            self.ui.textBrowser_status.append(
    ...
    def write_rec(self):
        ...
        self.rec_filename = self.ui.lineEdit_filenameSN.text().replace(
            ".tif", ".tif.rec"
        )
        ...
        if not os.path.isfile(self.rec_filepath):
            self.ui.textBrowser_status.setText(
